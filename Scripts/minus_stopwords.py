...
for dirpath, dirs, files in os.walk('../Files_Crawled'):
    for filename in fnmatch.filter(files, '*_crawled.xml_clean.xml'):
        with open('../Files_Crawled/'+filename, 'r') as originalfile, open('../Files_Working_Directory/'+filename[:-21]+'ohne_css_test.xml', 'w') as testfile:
...
for dirpath, dirs, files in os.walk('../Files_Working_Directory'):
    for filename in fnmatch.filter(files, '*_ohne_css_test.xml'):
        with open('../Files_Working_Directory/'+filename, 'r') as originalfile, open('../Files_3/'+filename[:-8]+'ohne_stop.xml', 'w') as testfile:
                for line in originalfile:
                    line = line.split()
                    for n in line:
                        if n in stopwords:
...
for dirpath, dirs, files in os.walk('../Files_Working_Directory'):
    for filename in fnmatch.filter(files, '*_ohne_css_test_ohne_stop.xml'):
        with open('../Files_Working_Directory/'+filename, 'r') as openfile, open('../Files_3/'+filename[:-27]+'test_three.xml', 'w') as newfile:
